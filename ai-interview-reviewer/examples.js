...
(function () {
  ...
  function codingCase(family, index, difficulty, title, prompt, variation, trap) {
    ...
    return {
      ...
      summary: family.summary + " " + variation,
      prompt: prompt,
      questions: family.questions,
      hints: family.hints,
      solution: family.solution + " For this variation, " + variation.toLowerCase(),
      code: family.code,
      artifactLabel: "Pseudocode",
  ...
  var codeFamilies = [
    ...
    },
    {
      ...
      code: "left, right = 0, len(values) - 1\nwhile left < right:\n    score = combine(values[left], values[right])\n    if score == target: return [left, right]\n    if score < target: left += 1\n    else: right -= 1",
    ...
    },
    {
      ...
      code: "invariant = establish_initial_state()\nwhile work_remains():\n    candidate = choose_next(invariant)\n    update(invariant, candidate)\n    assert_invariant(invariant)\nreturn extract_answer(invariant)",
    ...
    },
    {
      ...
      code: "dp = [base_case for _ in range(size + 1)]\nfor state in states_in_dependency_order:\n    dp[state] = best(transition(dp, state))\nreturn dp[target_state]",
